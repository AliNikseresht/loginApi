...
function HandleData() {
    ...
    const titleForgot = document.getElementById('set')
    ...
    titleForgot.title = maindata.titleForgot;
...
form.addEventListener("submit", function (event) {
    ...
    if (username.value === "kminchelle" && password.value === "0lelplR") {
        ...
        if (selectedLanguage === 'English') {
            ...
            alert("نام کاربری و گذرواژه صحیح وارد کنید!");
        } else if (selectedLanguage === 'Deutsch') {
            alert("Geben Sie einen gültigen Benutzernamen und ein gültiges Passwort ein!");
        }
    }
    ...
    fetch("https://dummyjson.com/auth/login", {
        ...
        .then((res) => res.json())
        .then((mydata) => {
            if (mydata && mydata.username === username.value && mydata.password === password.value) {
                window.location.href = "index-music.html"
            } else {
                if (selectedLanguage === 'English') {
                    alert("Enter Valid UserName & Password!");
